...
set(WEBRTC_PATCHES
    ...
    alchemy-0002-windows-media-audio-category.patch
    alchemy-0003-macos-reinit-on-format-change.patch
    alchemy-0004-upsample-to-48khz-for-aec.patch
    alchemy-0005-aec3-render-buffer-workaround.patch
    alchemy-0006-crash-on-fatal-error.patch
    alchemy-0007-keep-audio-processing-on-mute.patch
    # Behaviour the viewer relies on that lives in webrtc-sdk's fork rather
    # than in Linden's patch set.
    alchemy-0008-allow-audio-transport-while-active.patch
    alchemy-0009-init-recording-and-aec-render-reference.patch
    alchemy-0010-capture-format-before-first-sender.patch
    alchemy-0011-leave-platform-audio-processing-alone.patch
)
...
if(DEFINED VCPKG_BUILD_TYPE)
    set(WEBRTC_BUILD_CONFIGS "${VCPKG_BUILD_TYPE}")
else()
    set(WEBRTC_BUILD_CONFIGS debug release)
endif()

foreach(BUILD_CONFIG IN LISTS WEBRTC_BUILD_CONFIGS)
...
string(TOUPPER "${WEBRTC_BUILD_CONFIGS}" WEBRTC_IMPORTED_CONFIGURATIONS)
