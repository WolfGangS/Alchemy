...
#include "alareasearchmenu.h"
#include "alscrolllistctrl.h"
...
class ALAreaSearch::ALParcelChangeObserver : public LLParcelObserver
{
public:
    ALParcelChangeObserver(ALAreaSearch* area_search_floater) : mAreaSearchFloater(area_search_floater) {}
...
private:
    ...
    ALAreaSearch* mAreaSearchFloater;
};

class ALAreaSearchTouchTimer : public LLEventTimer
{
public:
    ALAreaSearchTouchTimer(const LLUUID& object_id, F32 timeout) :
    ...
    bool tick() override
    {
        ...
        if (objectp)
        {
            ALPanelAreaSearchList::touchObject(objectp);
        }
...
ALAreaSearch::ALAreaSearch(const LLSD& key) :
    LLFloater(key)
{
    gAgent.setALAreaSearchActive(true);
    ...
    mParcelChangedObserver = std::make_unique<ALParcelChangeObserver>(this);
    LLViewerParcelMgr::getInstance()->addObserver(mParcelChangedObserver.get());
}

ALAreaSearch::~ALAreaSearch()
{
    gAgent.setALAreaSearchActive(false);
    ...
    {
        LL_WARNS("ALAreaSearch") << "ALAreaSearch::~ALAreaSearch() failed to delete callback" << LL_ENDL;
    }
...
bool ALAreaSearch::postBuild()
{
    mTab = getChild<LLTabContainer>("area_searchtab");

    if (!gSavedSettings.getBOOL("AlchemyAreaSearchAdvanced"))
    {
    ...
    mRlvBehaviorCallbackConnection = gRlvHandler.setBehaviourCallback(boost::bind(&ALAreaSearch::updateRlvRestrictions, this, _1));

    return LLFloater::postBuild();
}

void ALAreaSearch::onOpen(const LLSD& key)
{
    mTab->selectTab(1);
}

void ALAreaSearch::draw()
{
    LLFloater::draw();
...
//static
void ALAreaSearch::idle(void* user_data)
{
    ALAreaSearch* self = (ALAreaSearch*)user_data;
    self->findObjects();
    self->processRequestQueue();
}

// static
void* ALAreaSearch::createPanelList(void* data)
{
    ALAreaSearch* self = (ALAreaSearch*)data;
    self->mPanelList = new ALPanelAreaSearchList(self);
    return self->mPanelList;
}

// static
void* ALAreaSearch::createPanelFind(void* data)
{
    ALAreaSearch* self = (ALAreaSearch*)data;
    self->mPanelFind = new ALPanelAreaSearchFind(self);
    return self->mPanelFind;
}

// static
void* ALAreaSearch::createPanelFilter(void* data)
{
    ALAreaSearch* self = (ALAreaSearch*)data;
    self->mPanelFilter = new ALPanelAreaSearchFilter(self);
    return self->mPanelFilter;
}

// static
void* ALAreaSearch::createPanelAdvanced(void* data)
{
    ALAreaSearch* self = (ALAreaSearch*)data;
    self->mPanelAdvanced = new ALPanelAreaSearchAdvanced();
    return self->mPanelAdvanced;
}

// static
void* ALAreaSearch::createPanelOptions(void* data)
{
    ALAreaSearch* self = (ALAreaSearch*)data;
    self->mPanelOptions = new ALPanelAreaSearchOptions(self);
    return self->mPanelOptions;
}

void ALAreaSearch::updateRlvRestrictions(ERlvBehaviour behavior)
{
...
void ALAreaSearch::refreshList(bool cache_clear)
{
    mActive = true;
    checkRegion();
    if (cache_clear)
...
void ALAreaSearch::findObjects()
{
    ...
    LL_DEBUGS("AlchemyAreaSearch_spammy") << "Doing a ALAreaSearch::findObjects" << LL_ENDL;
    ...
    for (S32 i = 0; i < object_count; i++)
    {
        ...
        if (object_id.isNull())
        {
            LL_WARNS("ALAreaSearch") << "WTF?! Selectable object with id of NULL!!" << LL_ENDL;
            continue;
        }
        ...
        {
            ALObjectProperties& details = mObjectDetails[object_id];
        ...
        {
            ALObjectProperties& details = mObjectDetails[object_id];
            if (details.request == ALObjectProperties::FINISHED)
            {
                matchObject(details, objectp);
            }

            if (details.request == ALObjectProperties::FAILED)
            {
                // object came back into view
                details.request = ALObjectProperties::NEED;
                details.local_id = objectp->getLocalID();
                details.region_handle = objectp->getRegion()->getHandle();
                mRequestNeedsSent = true;
                mRequested++;
    ...
    for (auto& object_it : mObjectDetails)
    {
        if (object_it.second.request == ALObjectProperties::NEED || object_it.second.request == ALObjectProperties::SENT)
        {
            const LLUUID& id = object_it.second.id;
            if (LLViewerObject* objectp = gObjectList.findObject(id); !objectp)
            {
                object_it.second.request = ALObjectProperties::FAILED;
                mRequested--;
            }
            else
            {
    ...
    {
        LL_DEBUGS("ALAreaSearch") << "Requested mismatch: " << request_count << " actual vs. " << mRequested << LL_ENDL;
...
bool ALAreaSearch::isSearchableObject(LLViewerObject* objectp, LLViewerRegion* our_region) const
{
...
void ALAreaSearch::processRequestQueue()
{
    ...
    {
        LL_DEBUGS("ALAreaSearch") << "Timeout reached, resending requests."<< LL_ENDL;
        S32 request_count = 0;
        S32 failed_count = 0;
        for (auto& object_it : mObjectDetails)
        {
            if (object_it.second.request == ALObjectProperties::SENT)
            {
                object_it.second.request = ALObjectProperties::NEED;
                mRequestNeedsSent = true;
                request_count++;
            }

            if (object_it.second.request == ALObjectProperties::FAILED)
            {
                failed_count++;
            }
        }
        ...
        {
            LL_DEBUGS("ALAreaSearch") << "No pending requests found."<< LL_ENDL;
        }
        else
        {
            LL_DEBUGS("ALAreaSearch") << request_count << " pending requests found."<< LL_ENDL;
        }

        LL_DEBUGS("ALAreaSearch") << failed_count << " failed requests found."<< LL_ENDL;
    }
    ...
    for (const auto regionp : LLWorld::getInstance()->getRegionList())
    {
        ...
        for (auto& object_it : mObjectDetails)
        {
            if (object_it.second.request == ALObjectProperties::NEED && object_it.second.region_handle == region_handle)
            {
                request_list.push_back(object_it.second.local_id);
                object_it.second.request = ALObjectProperties::SENT;
...
void ALAreaSearch::requestObjectProperties(const std::vector<U32>& request_list, bool select, LLViewerRegion* regionp)
{
    ...
    for (const auto& id : request_list)
    {
        ...
        {
            LL_DEBUGS("ALAreaSearch") << "Sent one full " << (select ? "ObjectSelect" : "ObjectDeselect") << " message with " << select_count << " object data blocks." << LL_ENDL;
    ...
    if (!start_new_message)
    {
        LL_DEBUGS("ALAreaSearch") << "Sent one partcial " << (select ? "ObjectSelect" : "ObjectDeselect") << " message with " << select_count << " object data blocks." << LL_ENDL;
        msg->sendReliable(regionp->getHost());
    }
}

void ALAreaSearch::processObjectProperties(LLMessageSystem* msg)
{
    if (!mActive)
    {
        return;
    ...
    LL_DEBUGS("ALAreaSearch")  << "Got processObjectProperties message with " << count << " object(s)" << LL_ENDL;
    for (S32 i = 0; i < count; i++)
    {
        ...
        if (object_id.isNull())
        {
            LL_WARNS("ALAreaSearch") << "Got Object Properties with NULL id" << LL_ENDL;
            continue;
        }
        ...
        if (details.request != ALObjectProperties::FINISHED)
        {
            ...
            details.request = ALObjectProperties::FINISHED;
            mLastPropertiesReceivedTimer.start();
            ...
            LL_DEBUGS("AlchemyAreaSearch_spammy") << "Got properties for object: " << object_id << LL_ENDL;
...
void ALAreaSearch::matchObject(ALObjectProperties& details, LLViewerObject* objectp)
{
    if (details.listed)
    {
    ...
    if (mRegexSearch)
    {
        ...
        {
            LL_WARNS_ONCE("ALAreaSearch") << "boost::regex_error error in regex: "<< e.what() << LL_ENDL;
        }
        catch(const std::exception& e)
        {
            LL_WARNS_ONCE("ALAreaSearch") << "std::exception error in regex: "<< e.what() << LL_ENDL;
        }
        catch (...)
        {
            LL_WARNS_ONCE("ALAreaSearch") << "Unknown error in regex" << LL_ENDL;
        }
    }
    else
    {
    ...
    if (details.sale_info.isForSale())
    {
        ...
        std::string cost_label = LLTrans::getString("AlchemyAreaSearch_Cost_Label", args);
...
void ALAreaSearch::updateObjectCosts(const LLUUID& object_id, F32 object_cost, F32 link_cost, F32 physics_cost, F32 link_physics_cost)
{
    if (!mActive)
    {
        return;
    ...
    if (ALScrollListCtrl* result_list = mPanelList->getResultList(); result_list)
    {
...
void ALAreaSearch::getNameFromUUID(const LLUUID& id, std::string& name, bool group, bool& name_requested)
{
    ...
    if (group)
    {
        ...
        if (!mNamesRequested.contains(id))
        {
            ...
            mNameCacheConnections.try_emplace(id, gCacheName->get(id, group, boost::bind(&ALAreaSearch::callbackLoadFullName, this, _1, _2))); // mNamesRequested will do the dupe check
        }
        name_requested = true;
    }
    else
    {
        if (LLAvatarName av_name; !LLAvatarNameCache::get(id, &av_name))
        {
            ...
            {
                mNamesRequested.emplace(id);
                mNameCacheConnections.try_emplace(id, LLAvatarNameCache::get(id, boost::bind(&ALAreaSearch::avatarNameCacheCallback, this, _1, _2))); // mNamesRequested will do the dupe check
            }
            name_requested = true;
        }
        else
...
void ALAreaSearch::avatarNameCacheCallback(const LLUUID& id, const LLAvatarName& av_name)
{
    callbackLoadFullName(id, av_name.getCompleteName());
}

void ALAreaSearch::callbackLoadFullName(const LLUUID& id, const std::string& full_name)
{
...
void ALAreaSearch::updateCounterText()
{
    LLStringUtil::format_map_t args;
...
void ALAreaSearch::onCommitLine()
{
...
bool ALAreaSearch::regexTest(std::string_view text)
{
    ...
    catch(boost::regex_error& e)
    {
        ...
        LL_DEBUGS("ALAreaSearch") << "boost::regex_error error in regex: "<< e.what() << LL_ENDL;
        return false;
    }
    catch(const std::exception& e)
    {
        ...
        args["EWHAT"] = e.what();
        LLNotificationsUtil::add("RegExFail", args);
        LL_DEBUGS("ALAreaSearch") << "std::exception error in regex: "<< e.what() << LL_ENDL;
        return false;
    }
    catch (...)
    {
        ...
        LLNotificationsUtil::add("RegExFail", args);
        LL_DEBUGS("ALAreaSearch") << "Unknown error in regex" << LL_ENDL;
        return false;
    }
...
void ALAreaSearch::clearSearchText()
{
    mSearchName.erase();
    mSearchDescription.erase();
    mSearchOwner.erase();
...
void ALAreaSearch::onButtonClickedSearch()
{
...
void ALAreaSearch::onCommitCheckboxRegex()
{
...
void ALAreaSearch::setFindOwnerText(const std::string& value)
{
...
ALPanelAreaSearchList::ALPanelAreaSearchList(ALAreaSearch* pointer)
:   LLPanel(),
    mALAreaSearch(pointer)
{
...
bool ALPanelAreaSearchList::postBuild()
{
    mResultList = getChild<ALScrollListCtrl>("result_list");
    mResultList->setDoubleClickCallback(boost::bind(&ALPanelAreaSearchList::onDoubleClick, this));
    mResultList->sortByColumn("name", true);
    mResultList->setContextMenu(&gALAreaSearchMenu);

    mCounterText = getChild<LLTextBox>("counter");

    mRefreshButton = getChild<LLButton>("Refresh");
    mRefreshButton->setClickedCallback(boost::bind(&ALPanelAreaSearchList::onClickRefresh, this));

    mCheckboxBeacons = getChild<LLCheckBoxCtrl>("beacons");
    mCheckboxBeacons->setCommitCallback(boost::bind(&ALPanelAreaSearchList::onCommitCheckboxBeacons, this));

    mAgentLastPosition = gAgent.getPositionGlobal();

    updateResultListColumns();
    mALAreaSearchColumnConfigConnection = gSavedSettings.getControl("AlchemyAreaSearchColumnConfig")->getSignal()->connect(boost::bind(&ALPanelAreaSearchList::updateResultListColumns, this));

    return LLPanel::postBuild();
}

// virtual
ALPanelAreaSearchList::~ALPanelAreaSearchList()
{
    if (mALAreaSearchColumnConfigConnection.connected())
    {
        mALAreaSearchColumnConfigConnection.disconnect();
    }
}

void ALPanelAreaSearchList::onClickRefresh()
{
    mALAreaSearch->refreshList(true);
}
...
// - Neremyn
bool ALPanelAreaSearchList::handleKeyHere(KEY key, MASK mask)
{
...
// - Neremyn
void ALPanelAreaSearchList::onCopyToClipboard()
{
    if (!mResultList)
    {
        LL_WARNS("ALAreaSearch") << "Result list control not found" << LL_ENDL;
        return;
    }
...
void ALPanelAreaSearchList::onCommitCheckboxBeacons()
{
    mALAreaSearch->setBeacons(mCheckboxBeacons->get());
}

void ALPanelAreaSearchList::setCounterText()
{
    mCounterText->setText(getString("ListedPendingTotalBlank"));
}

void ALPanelAreaSearchList::setCounterText(LLStringUtil::format_map_t args)
{
    mCounterText->setText(getString("ListedPendingTotalFilled", args));
}

void ALPanelAreaSearchList::onDoubleClick()
{
    ...
    {
        ALObjectProperties& details = mALAreaSearch->mObjectDetails[object_id];
        LLTracker::trackLocation(objectp->getPositionGlobal(), details.name, "", LLTracker::LOCATION_ITEM);

        if (mALAreaSearch->getPanelAdvanced()->mCheckboxClickBuy->get())
        {
            buyObject(details, objectp);
        }

        if (mALAreaSearch->getPanelAdvanced()->mCheckboxClickTouch->get())
        {
            touchObject(objectp);
        }

        if (mALAreaSearch->getPanelAdvanced()->mCheckboxClickSit->get())
        {
...
void ALPanelAreaSearchList::updateScrollList()
{
    ...
    for (const auto* item : items)
    {
        ...
        if (!objectp || !mALAreaSearch->isSearchableObject(objectp, our_region))
        {
            ...
            mALAreaSearch->mObjectDetails[row_id].listed = false ;
            deleted = true;
        }
        else
        {
...
void ALPanelAreaSearchList::updateResultListColumns()
{
    U32 column_config = gSavedSettings.getU32("AlchemyAreaSearchColumnConfig");
...
void ALPanelAreaSearchList::onColumnVisibilityChecked(const LLSD& userdata)
{
    ...
    gSavedSettings.setU32("AlchemyAreaSearchColumnConfig", new_value);

    updateResultListColumns();
}

bool ALPanelAreaSearchList::onEnableColumnVisibilityChecked(const LLSD& userdata)
{
    const std::string& column = userdata.asStringRef();
    U32 column_config = gSavedSettings.getU32("AlchemyAreaSearchColumnConfig");
...
void ALPanelAreaSearchList::updateName(const LLUUID& id, const std::string& name)
{
    ...
    {
        const LLUUID& row_id = item->getUUID();
        ALObjectProperties& details = mALAreaSearch->mObjectDetails[row_id];
...
bool ALPanelAreaSearchList::onContextMenuItemEnable(const LLSD& userdata)
{
...
bool ALPanelAreaSearchList::onContextMenuItemVisibleRLV(const LLSD& userdata)
{
...
bool ALPanelAreaSearchList::onContextMenuItemClick(const LLSD& userdata)
{
    const std::string& action = userdata.asStringRef();
    LL_DEBUGS("ALAreaSearch") << "Right click menu " << action << " was selected." << LL_ENDL;
    ...
    {
        mALAreaSearch->setFindOwnerText(gAgentUsername);
        mALAreaSearch->onButtonClickedSearch();
        return true;
    }
    ...
    case 't': // touch
    {
        ...
        for (const auto* item : selected)
        {
            ...
            case 't': // touch
            {
                new ALAreaSearchTouchTimer(item->getUUID(), cnt * 0.2f);
                cnt++;
            }
                break;
            default:
    ...
    case 'q': // q_zoom
    {
        if (mResultList->getNumSelected() == 1)
        {
            ...
            if (objectp)
            {
                ...
                {
                case 'b': // buy
                    buyObject(mALAreaSearch->mObjectDetails[object_id], objectp);
                    break;
                case 'p': // p_teleport
                    gAgent.teleportViaLocation(objectp->getPositionGlobal());
                    break;
                case 'u': // sit
                    sitOnObject(mALAreaSearch->mObjectDetails[object_id], objectp);
                    break;
                case 'q': // q_zoom
                {
        ...
        for (const auto* item : selected)
        {
            ...
            if (objectp)
            {
                ...
                if (c == 'r')
                {
                    ...
                    if (!mALAreaSearch || mALAreaSearch->mObjectDetails.end() == mALAreaSearch->mObjectDetails.find(object_id))
                    {
                        break;
                    }
...
// static
void ALPanelAreaSearchList::touchObject(LLViewerObject* objectp)
{
...
void ALPanelAreaSearchList::buyObject(ALObjectProperties& details, LLViewerObject* objectp)
{
    LLSelectMgr::getInstance()->deselectAll();
    LLSelectMgr::getInstance()->selectObjectAndFamily(objectp);
    ...
    {
        LL_WARNS("ALAreaSearch") << "No LLSelectNode node" << LL_ENDL;
    }
}

void ALPanelAreaSearchList::sitOnObject(ALObjectProperties& details, LLViewerObject* objectp)
{
...
ALPanelAreaSearchFind::ALPanelAreaSearchFind(ALAreaSearch* pointer)
:   LLPanel(),
    mALAreaSearch(pointer)
{
}

bool ALPanelAreaSearchFind::postBuild()
{
    mNameLineEditor = getChild<LLLineEditor>("name_search");
    mNameLineEditor->setCommitCallback(boost::bind(&ALAreaSearch::onCommitLine, mALAreaSearch));

    mDescriptionLineEditor = getChild<LLLineEditor>("description_search");
    mDescriptionLineEditor->setCommitCallback(boost::bind(&ALAreaSearch::onCommitLine, mALAreaSearch));

    mOwnerLineEditor = getChild<LLLineEditor>("owner_search");
    mOwnerLineEditor->setCommitCallback(boost::bind(&ALAreaSearch::onCommitLine, mALAreaSearch));

    mGroupLineEditor = getChild<LLLineEditor>("group_search");
    mGroupLineEditor->setCommitCallback(boost::bind(&ALAreaSearch::onCommitLine, mALAreaSearch));

    mCreatorLineEditor = getChild<LLLineEditor>("creator_search");
    mCreatorLineEditor->setCommitCallback(boost::bind(&ALAreaSearch::onCommitLine, mALAreaSearch));

    mLastOwnerLineEditor = getChild<LLLineEditor>("last_owner_search");
    mLastOwnerLineEditor->setCommitCallback(boost::bind(&ALAreaSearch::onCommitLine, mALAreaSearch));

    mCheckboxRegex = getChild<LLCheckBoxCtrl>("regular_expression");
    mCheckboxRegex->setCommitCallback(boost::bind(&ALAreaSearch::onCommitCheckboxRegex, mALAreaSearch));

    mSearchButton = getChild<LLButton>("search");
    mSearchButton->setClickedCallback(boost::bind(&ALAreaSearch::onButtonClickedSearch, mALAreaSearch));

    mClearButton = getChild<LLButton>("clear");
    mClearButton->setClickedCallback(boost::bind(&ALPanelAreaSearchFind::onButtonClickedClear, this));

    return LLPanel::postBuild();
}

void ALPanelAreaSearchFind::onButtonClickedClear()
{
    ...
    mALAreaSearch->clearSearchText();
}

// handle the "enter" key
bool ALPanelAreaSearchFind::handleKeyHere(KEY key, MASK mask)
{
    if (KEY_RETURN == key)
    {
        mALAreaSearch->onButtonClickedSearch();
...
ALPanelAreaSearchFilter::ALPanelAreaSearchFilter(ALAreaSearch* pointer)
:   LLPanel(),
    mALAreaSearch(pointer)
{
}

bool ALPanelAreaSearchFilter::postBuild()
{
    mCheckboxLocked = getChild<LLCheckBoxCtrl>("filter_locked");
    mCheckboxLocked->set(gSavedSettings.getBOOL("AlchemyAreaSearch_OnlyLocked"));
    mCheckboxLocked->setCommitCallback(boost::bind(&ALPanelAreaSearchFilter::onCommitCheckbox, this));

    mCheckboxPhysical = getChild<LLCheckBoxCtrl>("filter_physical");
    mCheckboxPhysical->set(gSavedSettings.getBOOL("AlchemyAreaSearch_OnlyPhysical"));
    mCheckboxPhysical->setCommitCallback(boost::bind(&ALPanelAreaSearchFilter::onCommitCheckbox, this));

    mCheckboxTemporary = getChild<LLCheckBoxCtrl>("filter_temporary");
    mCheckboxTemporary->set(gSavedSettings.getBOOL("AlchemyAreaSearch_OnlyTemporary"));
    mCheckboxTemporary->setCommitCallback(boost::bind(&ALPanelAreaSearchFilter::onCommitCheckbox, this));

    mCheckboxPhantom = getChild<LLCheckBoxCtrl>("filter_phantom");
    mCheckboxPhantom->set(gSavedSettings.getBOOL("AlchemyAreaSearch_OnlyPhantom"));
    mCheckboxPhantom->setCommitCallback(boost::bind(&ALPanelAreaSearchFilter::onCommitCheckbox, this));

    mCheckboxForSale = getChild<LLCheckBoxCtrl>("filter_for_sale");
    mCheckboxForSale->set(gSavedSettings.getBOOL("AlchemyAreaSearch_FilterForSale"));
    mCheckboxForSale->setCommitCallback(boost::bind(&ALPanelAreaSearchFilter::onCommitCheckbox, this));

    mCheckboxAttachment = getChild<LLCheckBoxCtrl>("filter_attachment");
    mCheckboxAttachment->set(gSavedSettings.getBOOL("AlchemyAreaSearch_OnlyAttachments"));
    mCheckboxAttachment->setCommitCallback(boost::bind(&ALPanelAreaSearchFilter::onCommitCheckbox, this));

    mSpinForSaleMinValue = getChild<LLSpinCtrl>("min_price");
    mSpinForSaleMinValue->set((F32)gSavedSettings.getS32("AlchemyAreaSearch_MinimumPrice"));
    mSpinForSaleMinValue->setCommitCallback(boost::bind(&ALPanelAreaSearchFilter::onCommitSpin, this));

    mSpinForSaleMaxValue = getChild<LLSpinCtrl>("max_price");
    mSpinForSaleMaxValue->set((F32)gSavedSettings.getS32("AlchemyAreaSearch_MaximumPrice"));
    mSpinForSaleMaxValue->setCommitCallback(boost::bind(&ALPanelAreaSearchFilter::onCommitSpin, this));

    mComboClickAction = getChild<LLComboBox>("click_action");
    mComboClickAction->setValue(gSavedSettings.getLLSD("AlchemyAreaSearch_ClickAction"));
    mComboClickAction->setCommitCallback(boost::bind(&ALPanelAreaSearchFilter::onCommitCombo, this));

    mCheckboxExcludeAttachment = getChild<LLCheckBoxCtrl>("exclude_attachment");
    mCheckboxExcludeAttachment->set(gSavedSettings.getBOOL("AlchemyAreaSearch_ExcludeAttachments"));
    mCheckboxExcludeAttachment->setCommitCallback(boost::bind(&ALPanelAreaSearchFilter::onCommitCheckbox, this));

    mCheckboxExcludePhysics = getChild<LLCheckBoxCtrl>("exclude_physical");
    mCheckboxExcludePhysics->set(gSavedSettings.getBOOL("AlchemyAreaSearch_ExcludePhysical"));
    mCheckboxExcludePhysics->setCommitCallback(boost::bind(&ALPanelAreaSearchFilter::onCommitCheckbox, this));

    mCheckboxExcludetemporary = getChild<LLCheckBoxCtrl>("exclude_temporary");
    mCheckboxExcludetemporary->set(gSavedSettings.getBOOL("AlchemyAreaSearch_ExcludeTemporary"));
    mCheckboxExcludetemporary->setCommitCallback(boost::bind(&ALPanelAreaSearchFilter::onCommitCheckbox, this));

    mCheckboxExcludeReflectionProbes = getChild<LLCheckBoxCtrl>("exclude_reflection_probes");
    mCheckboxExcludeReflectionProbes->set(gSavedSettings.getBOOL("AlchemyAreaSearch_ExcludeReflectionProbes"));
    mCheckboxExcludeReflectionProbes->setCommitCallback(boost::bind(&ALPanelAreaSearchFilter::onCommitCheckbox, this));

    mCheckboxExcludeChildPrim = getChild<LLCheckBoxCtrl>("exclude_childprim");
    mCheckboxExcludeChildPrim->set(gSavedSettings.getBOOL("AlchemyAreaSearch_ExcludeChildPrims"));
    mCheckboxExcludeChildPrim->setCommitCallback(boost::bind(&ALPanelAreaSearchFilter::onCommitCheckbox, this));

    mCheckboxExcludeNeighborRegions = getChild<LLCheckBoxCtrl>("exclude_neighbor_region");
    mCheckboxExcludeNeighborRegions->set(gSavedSettings.getBOOL("AlchemyAreaSearch_ExcludeNeighborRegions"));
    mCheckboxExcludeNeighborRegions->setCommitCallback(boost::bind(&ALPanelAreaSearchFilter::onCommitCheckbox, this));

    mButtonApply = getChild<LLButton>("apply");
    mButtonApply->setClickedCallback(boost::bind(&ALAreaSearch::onButtonClickedSearch, mALAreaSearch));

    mButtonApply = getChild<LLButton>("save_as_default");
    mButtonApply->setClickedCallback(boost::bind(&ALPanelAreaSearchFilter::onButtonClickedSaveAsDefault, this));

    mCheckboxDistance = getChild<LLCheckBoxCtrl>("filter_distance");
    mCheckboxDistance->set(gSavedSettings.getBOOL("AlchemyAreaSearch_FilterDistance"));
    mCheckboxDistance->setCommitCallback(boost::bind(&ALPanelAreaSearchFilter::onCommitCheckbox, this));

    mSpinDistanceMinValue = getChild<LLSpinCtrl>("min_distance");
    mSpinDistanceMinValue->set((F32)gSavedSettings.getS32("AlchemyAreaSearch_MinimumDistance"));
    mSpinDistanceMinValue->setCommitCallback(boost::bind(&ALPanelAreaSearchFilter::onCommitSpin, this));

    mSpinDistanceMaxValue = getChild<LLSpinCtrl>("max_distance");
    mSpinDistanceMaxValue->set((F32)gSavedSettings.getS32("AlchemyAreaSearch_MaximumDistance"));
    mSpinDistanceMaxValue->setCommitCallback(boost::bind(&ALPanelAreaSearchFilter::onCommitSpin, this));

    mCheckboxMoaP = getChild<LLCheckBoxCtrl>("filter_moap");
    mCheckboxMoaP->set(gSavedSettings.getBOOL("AlchemyAreaSearch_OnlyMOAP"));
    mCheckboxMoaP->setCommitCallback(boost::bind(&ALPanelAreaSearchFilter::onCommitCheckbox, this));

    mCheckboxReflectionProbe = getChild<LLCheckBoxCtrl>("filter_reflection_probe");
    mCheckboxReflectionProbe->set(gSavedSettings.getBOOL("AlchemyAreaSearch_OnlyReflectionProbes"));
    mCheckboxReflectionProbe->setCommitCallback(boost::bind(&ALPanelAreaSearchFilter::onCommitCheckbox, this));

    mCheckboxPermCopy = getChild<LLCheckBoxCtrl>("filter_perm_copy");
    mCheckboxPermCopy->set(gSavedSettings.getBOOL("AlchemyAreaSearch_OnlyCopiable"));
    mCheckboxPermCopy->setCommitCallback(boost::bind(&ALPanelAreaSearchFilter::onCommitCheckbox, this));

    mCheckboxPermModify = getChild<LLCheckBoxCtrl>("filter_perm_modify");
    mCheckboxPermModify->set(gSavedSettings.getBOOL("AlchemyAreaSearch_OnlyModifiable"));
    mCheckboxPermModify->setCommitCallback(boost::bind(&ALPanelAreaSearchFilter::onCommitCheckbox, this));

    mCheckboxPermTransfer = getChild<LLCheckBoxCtrl>("filter_perm_transfer");
    mCheckboxPermModify->set(gSavedSettings.getBOOL("AlchemyAreaSearch_OnlyTransferable"));
    mCheckboxPermTransfer->setCommitCallback(boost::bind(&ALPanelAreaSearchFilter::onCommitCheckbox, this));

    mCheckboxAgentParcelOnly = getChild<LLCheckBoxCtrl>("filter_agent_parcel_only");
    mCheckboxAgentParcelOnly->set(gSavedSettings.getBOOL("AlchemyAreaSearch_OnlyCurrentParcel"));
    mCheckboxAgentParcelOnly->setCommitCallback(boost::bind(&ALPanelAreaSearchFilter::onCommitCheckbox, this));
...
void ALPanelAreaSearchFilter::onCommitCheckbox()
{
    mALAreaSearch->setFilterLocked(mCheckboxLocked->get());
    mALAreaSearch->setFilterPhantom(mCheckboxPhantom->get());
    mALAreaSearch->setFilterMoaP(mCheckboxMoaP->get());
    mALAreaSearch->setFilterReflectionProbe(mCheckboxReflectionProbe->get());

    if (mCheckboxExcludePhysics->get())
    {
        mALAreaSearch->setFilterPhysical(false);
        mCheckboxPhysical->set(false);
        mCheckboxPhysical->setEnabled(false);
        mALAreaSearch->setExcludePhysics(true);
    }
    else
    {
        mCheckboxPhysical->setEnabled(true);
        mALAreaSearch->setExcludePhysics(false);
    }
    mALAreaSearch->setFilterPhysical(mCheckboxPhysical->get());

    if (mCheckboxExcludetemporary->get())
    {
        mALAreaSearch->setFilterTemporary(false);
        mCheckboxTemporary->set(false);
        mCheckboxTemporary->setEnabled(false);
        mALAreaSearch->setExcludetemporary(true);
    }
    else
    {
        mCheckboxTemporary->setEnabled(true);
        mALAreaSearch->setExcludetemporary(false);
    }
    mALAreaSearch->setFilterTemporary(mCheckboxTemporary->get());

    if (mCheckboxExcludeReflectionProbes->get())
    {
        mALAreaSearch->setFilterReflectionProbe(false);
        mCheckboxReflectionProbe->set(false);
        mCheckboxReflectionProbe->setEnabled(false);
        mALAreaSearch->setExcludeReflectionProbe(true);
    }
    else
    {
        mCheckboxReflectionProbe->setEnabled(true);
        mALAreaSearch->setExcludeReflectionProbe(false);
    }

    if (mCheckboxExcludeAttachment->get())
    {
        mALAreaSearch->setFilterAttachment(false);
        mCheckboxAttachment->set(false);
        mCheckboxAttachment->setEnabled(false);
        mALAreaSearch->setExcludeAttachment(true);
    }
    else
    {
        mCheckboxAttachment->setEnabled(true);
        mALAreaSearch->setExcludeAttachment(false);
    }
    mALAreaSearch->setFilterAttachment(mCheckboxAttachment->get());

    mALAreaSearch->setFilterForSale(mCheckboxForSale->get());
    mSpinForSaleMinValue->setEnabled(mCheckboxForSale->get());
    mSpinForSaleMaxValue->setEnabled(mCheckboxForSale->get());

    mALAreaSearch->setFilterDistance(mCheckboxDistance->get());
    mSpinDistanceMinValue->setEnabled(mCheckboxDistance->get());
    mSpinDistanceMaxValue->setEnabled(mCheckboxDistance->get());

    mALAreaSearch->setExcludeChildPrims(mCheckboxExcludeChildPrim->get());

    mALAreaSearch->setExcludeNeighborRegions(mCheckboxExcludeNeighborRegions->get());

    mALAreaSearch->setFilterPermCopy(mCheckboxPermCopy->get());
    mALAreaSearch->setFilterPermModify(mCheckboxPermModify->get());
    mALAreaSearch->setFilterPermTransfer(mCheckboxPermTransfer->get());

    mALAreaSearch->setFilterAgentParcelOnly(mCheckboxAgentParcelOnly->get());
}

void ALPanelAreaSearchFilter::onCommitSpin()
{
    mALAreaSearch->setFilterForSaleMin(mSpinForSaleMinValue->getValue().asInteger());
    mALAreaSearch->setFilterForSaleMax(mSpinForSaleMaxValue->getValue().asInteger());

    mALAreaSearch->setFilterDistanceMin(mSpinDistanceMinValue->getValue().asInteger());
    mALAreaSearch->setFilterDistanceMax(mSpinDistanceMaxValue->getValue().asInteger());
}

void ALPanelAreaSearchFilter::onCommitCombo()
{
    if (mComboClickAction->getCurrentIndex() > 0)
    {
        mALAreaSearch->setFilterClickAction(true);
        mALAreaSearch->setFilterClickActionType((U8)mComboClickAction->getCurrentIndex());
    }
    else
    {
        mALAreaSearch->setFilterClickAction(false);
        mALAreaSearch->setFilterClickActionType(0);
    }
}

void ALPanelAreaSearchFilter::onButtonClickedSaveAsDefault()
{
    gSavedSettings.setBOOL("AlchemyAreaSearch_OnlyLocked", mCheckboxLocked->get());
    gSavedSettings.setBOOL("AlchemyAreaSearch_OnlyPhysical", mCheckboxPhysical->get());
    gSavedSettings.setBOOL("AlchemyAreaSearch_OnlyTemporary", mCheckboxTemporary->get());
    gSavedSettings.setBOOL("AlchemyAreaSearch_OnlyPhantom", mCheckboxPhantom->get());
    gSavedSettings.setBOOL("AlchemyAreaSearch_FilterForSale", mCheckboxForSale->get());
    gSavedSettings.setBOOL("AlchemyAreaSearch_OnlyAttachments", mCheckboxAttachment->get());
    gSavedSettings.setS32("AlchemyAreaSearch_MinimumPrice", (S32)mSpinForSaleMinValue->get());
    gSavedSettings.setS32("AlchemyAreaSearch_MaximumPrice", (S32)mSpinForSaleMaxValue->get());
    gSavedSettings.setLLSD("AlchemyAreaSearch_ClickAction", mComboClickAction->getValue());
    gSavedSettings.setBOOL("AlchemyAreaSearch_ExcludeAttachments", mCheckboxExcludeAttachment->get());
    gSavedSettings.setBOOL("AlchemyAreaSearch_ExcludePhysical", mCheckboxExcludePhysics->get());
    gSavedSettings.setBOOL("AlchemyAreaSearch_ExcludeTemporary", mCheckboxExcludetemporary->get());
    gSavedSettings.setBOOL("AlchemyAreaSearch_ExcludeReflectionProbes", mCheckboxExcludeReflectionProbes->get());
    gSavedSettings.setBOOL("AlchemyAreaSearch_ExcludeChildPrims", mCheckboxExcludeChildPrim->get());
    gSavedSettings.setBOOL("AlchemyAreaSearch_ExcludeNeighborRegions", mCheckboxExcludeNeighborRegions->get());
    gSavedSettings.setBOOL("AlchemyAreaSearch_FilterDistance", mCheckboxDistance->get());
    gSavedSettings.setS32("AlchemyAreaSearch_MinimumDistance", (S32)mSpinDistanceMinValue->get());
    gSavedSettings.setS32("AlchemyAreaSearch_MaximumDistance", (S32)mSpinDistanceMaxValue->get());
    gSavedSettings.setBOOL("AlchemyAreaSearch_OnlyMOAP", mCheckboxMoaP->get());
    gSavedSettings.setBOOL("AlchemyAreaSearch_OnlyReflectionProbes", mCheckboxReflectionProbe->get());
    gSavedSettings.setBOOL("AlchemyAreaSearch_OnlyCopiable", mCheckboxPermCopy->get());
    gSavedSettings.setBOOL("AlchemyAreaSearch_OnlyModifiable", mCheckboxPermModify->get());
    gSavedSettings.setBOOL("AlchemyAreaSearch_OnlyTransferable", mCheckboxPermModify->get());
    gSavedSettings.setBOOL("AlchemyAreaSearch_OnlyCurrentParcel", mCheckboxAgentParcelOnly->get());
}
...
ALPanelAreaSearchOptions::ALPanelAreaSearchOptions(ALAreaSearch* pointer)
:   LLPanel(),
    mALAreaSearch(pointer)
{
    mCommitCallbackRegistrar.add("AreaSearch.DisplayColumn", boost::bind(&ALPanelAreaSearchOptions::onCommitCheckboxDisplayColumn, this, _2));
    mEnableCallbackRegistrar.add("AreaSearch.EnableColumn", boost::bind(&ALPanelAreaSearchOptions::onEnableColumnVisibilityChecked, this, _2));
}

void ALPanelAreaSearchOptions::onCommitCheckboxDisplayColumn(const LLSD& userdata)
{
    const std::string& column_name = userdata.asStringRef();
    if (column_name.empty())
    {
        LL_WARNS("ALAreaSearch") << "Missing action text." << LL_ENDL;
        return;
    }

    mALAreaSearch->getPanelList()->onColumnVisibilityChecked(userdata);
}

bool ALPanelAreaSearchOptions::onEnableColumnVisibilityChecked(const LLSD& userdata)
{
    return mALAreaSearch->getPanelList()->onEnableColumnVisibilityChecked(userdata);
}
...
bool ALPanelAreaSearchAdvanced::postBuild()
{
